...
class SLAUApp:
    ...
    def create_widgets(self):
        ...
        self.btn_worker1 = ttk.Button(
            worker_frame,
            text="🖥️ Worker 5001",
            command=self.start_worker1,
            width=24
        )
        self.btn_worker1.pack(pady=2)

        self.btn_worker2 = ttk.Button(
            worker_frame,
            text="🖥️ Worker 5002",
            command=self.start_worker2,
            width=24
        )
        self.btn_worker2.pack(pady=2)

        self.btn_stop_workers = ttk.Button(
            worker_frame,
            text="⏹️ Остановить workers",
        ...
        self.btn_master = ttk.Button(
            main_ops_frame,
            text="🚀 2. Запустить master.py",
            command=self.run_master,
            width=28
        )
        ...
        self.btn_test_orth = ttk.Button(
            test_frame,
            text="🧪 Тест MGS",
            command=self.run_test_orth,
            width=26
        )
        self.btn_test_orth.pack(pady=2)

        self.btn_test_io = ttk.Button(
            test_frame,
            text="🧪 Тест IO Utils",
            command=self.run_test_io,
            width=26
        )
        self.btn_test_io.pack(pady=2)

        self.btn_test_load = ttk.Button(
            test_frame,
            text="🧪 Тест Load",
            command=self.run_test_load,
            width=26
        )
        self.btn_test_load.pack(pady=2)

        self.btn_test_dist = ttk.Button(
            test_frame,
            text="🧪 Тест Distributed",
            command=self.run_test_dist,
            width=26
        )
    ...
    def update_run_buttons(self):
        ...
        self.btn_test_load.config(state=state)
    ...
    def stop_current_process(self):
        if self.current_process and self.current_process.poll() is None:
            try:
                ...
                self.log("[СТОП] Процесс остановлен пользователем")
    ...
    def run_command_live(self, cmd, title=None, cwd=None):
        if self.is_running:
            messagebox.showwarning(
                "Внимание",
                "Сначала дождитесь завершения текущей задачи или остановите её."
            )
            return
                ...
                for line in iter(process.stdout.readline, ''):
                    if line:
                        ...
                        if (
                            "шаг" in low or
                            "test" in low or
                            "размерность" in low or
                            "benchmark" in low or
                            "passed" in low or
                            "failed" in low
                        ):
                            self.progress_var.set(f"▶ {clean_line[:80]}")
    ...
    def _start_worker(self, port, tag, proc_attr):
        ...
        try:
            popen_kwargs = dict(
                args=[self.python_exec, "worker.py", "--port", str(port)],
            ...
            threading.Thread(
                target=self._read_worker_output_live,
                args=(proc, tag),
                daemon=True
            ).start()
        ...
        for i, proc in enumerate([self.worker1_proc, self.worker2_proc], 1):
            if proc and proc.poll() is None:
                try:
                    ...
                    self.log(f"[ОШИБКА] Не удалось остановить Worker {5000 + i}: {e}")
    ...
    def run_master(self):
        ...
        if not self.workers_ready():
            messagebox.showwarning(
                ...
                "Сначала запустите Worker 5001 и Worker 5002."
            )
            return
    ...
    def _run_pytest_file(self, test_file, title):
        ...
        # ВАЖНО: -s нужен, чтобы были видны print(...) из тестов
        cmd = [self.python_exec, "-m", "pytest", test_file, "-v", "-s"]
        self.run_command_live(cmd, title=title)

    def run_test_orth(self):
        self._run_pytest_file(
            os.path.join("tests", "test_orth_solver.py"),
            "МОДУЛЬНЫЙ ТЕСТ: MGS / ORTH_SOLVER"
        )

    def run_test_io(self):
        self._run_pytest_file(
            os.path.join("tests", "test_io_utils.py"),
            "МОДУЛЬНЫЙ ТЕСТ: IO_UTILS"
        )

    def run_test_load(self):
        self._run_pytest_file(
            os.path.join("tests", "test_load.py"),
            "МОДУЛЬНЫЙ ТЕСТ: LOAD / BENCHMARK"
        )

    def run_test_dist(self):
        self._run_pytest_file(
            os.path.join("tests", "test_distributed.py"),
            "МОДУЛЬНЫЙ ТЕСТ: DISTRIBUTED"
        )
    ...
    def run_all_tests(self):
        ...
        # ВАЖНО: -s нужен для live-логов print(...) из всех тестов
        cmd = [self.python_exec, "-m", "pytest", tests_dir, "-v", "-s"]
